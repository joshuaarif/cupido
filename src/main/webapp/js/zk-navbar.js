							...
							.override(
									...
									_navbar,
									{
										...
										bind_ : function(subclass) {
											...
											jq(this.$n('cave')).addClass(
													'nav navbar-nav');
										},
										redraw : function(out) {

											var uuid = this.uuid;
											out.push('<nav ', this.domAttrs_(),
													'><div class="', this
															.$s('collapse'),
													'"><ul id="', uuid,
													'-cave" >');
					...
					zk.override(zkmax.nav.Navitem.prototype, _navitem, {
						...
						onFloatUp : function(ctl) {
							...
							for (var floatFound; wgt; wgt = wgt.parent) {
								...
								if (wgt == this.parent
										&& wgt.ignoreDescendantFloatUp_(this))
									return;
							...
							.override(
									...
									_nav,
									{
										...
										$s : function(subclass) {
											...
											return _nav.$s.apply(this,
													arguments);
										},

										domContent_ : function() {
											var label = '<span class="'
													+ this.$s('text')
													+ '">'
													+ (zUtl.encodeXML(this
															.getLabel()))
													+ '</span>', img = this
													.getImage(), iconSclass = this
													.domIcon_();
											...
											var info = this._detailed ? '<span id="'
													+ this.uuid
													+ '-info" class="'
													+ this.$s('info')
													+ '">'
